...
8.6. **新建 tab 补足 10 行**：当本 tab 这次写的素材**全部为 no_history**（即首次同步、KPI 上素材数 < 10 的新建 tab），且优化区域内 `原方案` + `新增` 行总数 < `MIN_ASSET_ROWS_FOR_NEW_SHEET`（默认 10），脚本在最后一个非空优化行之后追加 `(10 - 现有数)` 个 `新增` 占位行：
   - **写入**：`Asset type`（沿用最后一个 `原方案` 行的值）、`优化轮次`（固定为 `新增`）
   - **留空**：所有其它列（Asset、翻译、优化思路、字符数、Performance、Cost排序、Ctr、数据周期）
   - 格式与 dataValidation 从最后一个 `原方案` 行复制（PASTE_FORMAT + PASTE_DATA_VALIDATION），保证字体颜色和 Performance dropdown 一致
   - 不为 `新增` 行配对空白优化行（独立占位行）
   - kept tab（含 with_history 插入）**不进入**此分支，无论现有行数多少
   - 报告中通过 `padded_new_rows` 字段计数
9. 保留格式与验证：
   - 从最近的匹配参考行复制行格式
   - 随格式一并复制数据验证
   - 保留 `Performance` 验证
