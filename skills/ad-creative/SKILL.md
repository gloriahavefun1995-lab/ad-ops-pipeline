...
**Ask for the market listing link first if missing:**
For every task, if the user has not provided the app's market listing link, ask for it before proceeding.
Accepted links include:
- Google Play listing URL
- App Store listing URL

Use the market link to understand the app's positioning, core features, and store-facing messaging before writing ad creative.
...
Apply these counting rules to all ad copy in this skill unless a platform explicitly requires a different method.

- For `zh_CN`, `zh_TW`,  `ja`, and `ko`, each CJK character counts as **2 characters**
- For all other languages, use normal character counting
- Always calculate character counts exactly before delivering output
- If any line exceeds the limit after applying the correct counting rule, rewrite it instead of estimating or leaving it over the limit
- When reporting counts, use the final adjusted count that matches these rules

Examples:
- `你好世界` = 8 characters
- `春のセール` = 10 characters
- `봄 세일` =7 characters
- `Save More Today` = 15 characters
...
Avoid:
- synonym swaps or near-duplicates
- repeating the same sentence pattern across variations

For non-English creative:
- write directly in the target language
- do not draft in English and translate literally
- ensure each variation sounds locally written

### Step 3: Validate Against Specs

Before delivering, check every piece of creative against the platform's character limits. Flag anything that's over and provide a trimmed alternative.

- For `zh_CN`, `zh_TW`, `ja`, and `ko`, count each character as 2 characters
- For all other languages, use normal character counting
- Apply the correct counting rule before finalizing any headline, description, primary text, or other ad copy
...
- **Avoids** duplicates, near-duplicates, and simple rewordings across the same Sheet, other Ad Group Sheets in the same Campaign, and the current batch
- **Avoids** drafting in English first and translating for non-English creative; write directly in the target language
...
- **Reusing or lightly rewording existing creative** — each variation should be meaningfully distinct and written natively in the target language
